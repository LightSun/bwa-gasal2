...
{
	pem_opt_t *opt;
    opt = (pem_opt_t*)calloc(1, sizeof(pem_opt_t));
...
int bwa_pemerge(const pem_opt_t *opt, bseq1_t x[2])
{
    ...
    s[0] = (uint8_t*)malloc(x[0].l_seq);
    q[0] = (uint8_t*)malloc(x[0].l_seq);
    s[1] = (uint8_t*)malloc(x[1].l_seq);
    q[1] = (uint8_t*)malloc(x[1].l_seq);
    ...
    seq = (uint8_t*)malloc(l_seq + 1);
    qual = (uint8_t*)malloc(l_seq + 1);
