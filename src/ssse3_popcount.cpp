...
uint32_t popcnt(uint8_t* buffer, int chunks16) {
	uint32_t result = 0;
...
uint32_t simplecnt(uint8_t *buffer, int chunks16) {
	uint32_t result = 0;
...
//char verify(int index, uint32_t (*func)(uint64_t*, int), uint64_t *buffer, int chunks8, unsigned popcount_ref) {
//	char failed;
...
int main(int argc, char* argv[]) {
	...
	switch (function) {
		case 0:
			...
			failed |= verify(2, &sse2_popcount1, buffer, chunks_count, popcount_ref);
			failed |= verify(3, &sse2_popcount2, buffer, chunks_count, popcount_ref);
			failed |= verify(4, &ssse3_popcount1, buffer, chunks_count, popcount_ref);
			failed |= verify(5, &ssse3_popcount2, buffer, chunks_count, popcount_ref);
			failed |= verify(6, &ssse3_popcount3, buffer, chunks_count, popcount_ref);
			failed |= verify(7, &sse2_popcount3, buffer, chunks_count, popcount_ref);
			failed |= verify(8, &popcnt, buffer, chunks_count, popcount_ref);
			failed |= verify(9, &simplecnt, buffer, chunks_count, popcount_ref);
		...
		case 8:
			while (repeat_count--)
				popcnt(buffer, chunks_count);
			break;
		case 9:
			while (repeat_count--)
				simplecnt(buffer, chunks_count);
			break;
	}
