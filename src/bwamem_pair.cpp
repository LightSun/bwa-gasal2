...
void mem_pestat(const mem_opt_t *opt, int64_t l_pac, int n, const mem_alnreg_v *regs, mem_pestat_t pes[4])
{
	...
	for (i = 0; i < n>>1; ++i) {
		...
		//fprintf(stderr, "Error occured at pair no. %d, read 1\n", i);
		//fflush(stderr);
		if (cal_sub(opt, r[0]) > MIN_RATIO * r[0]->a[0].score) continue;
		//fprintf(stderr, "Error occured at pair no. %d, read 2\n", i);
		//fflush(stderr);
...
int mem_matesw(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_pestat_t pes[4], const mem_alnreg_t *a, int l_ms, const uint8_t *ms, mem_alnreg_v *ma)
{
	...
	for (r = 0; r < 4; ++r) {
		...
		if (a->rid == rid && re - rb >= opt->min_seed_len) { // no funny things happening
			...
			aln = ksw_align2(l_ms, seq, re - rb, ref, 5, opt->mat, opt->o_del, opt->e_del, opt->o_ins, opt->e_ins, xtra, 0, opt->use_avx2);
...
int mem_sam_pe(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_pestat_t pes[4], uint64_t id, bseq1_t s[2], mem_alnreg_v a[2])
{
	...
	// J.L. 2019-01-10 09:40 removed proto
